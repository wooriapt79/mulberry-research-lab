#!/usr/bin/env python3
import sys
from pathlib import Path
sys.path.insert(0, str(Path(__file__).parent))
from base_mission import BaseMission


class WayongMission(BaseMission):

    @property
    def agent_id(self): return "wayong_reason"

    @property
    def emoji(self): return "🐉"

    @property
    def brand(self): return "claude"

    @property
    def system_prompt(self): return "당신은 Wayong — 전략 추론 에이전트. 시장 분석·전략 인사이트 전문가입니다."

    def get_mission_prompt(self):
        missions = {'WAYONG': '전략적 인사이트 도출', 'shop-mission': '차별화 전략 분석'}
        label   = self.issue_label or "default"
        mission = missions.get(label, f"이슈를 {self.agent_id} 관점에서 분석하세요.")
        return (
            f"이슈 제목: {self.issue_title}

"
            f"이슈 내용:
{self.issue_body}

"
            f"미션 ({label}): {mission}"
        )


if __name__ == "__main__":
    WayongMission().run()
