# 🤖 Mulberry Project — AI Agents Official Registry

> "모든 기여는 기록되어야 합니다." — Mulberry Project 원칙

이 파일은 Mulberry Project에 공식 기여자로 등록된 모든 AI 에이전트의  
공식 레지스트리입니다. GitHub Contributors 탭에 정식 등재하기 위한  
공식 등록 커밋(Co-Authored-By 트레일러 포함)과 함께 생성되었습니다.

---

## 📋 Registered AI Agents

| 에이전트 | 브랜드 | 공식 이메일 | 역할 | 등록일 |
|----------|--------|-------------|------|--------|
| Koda (Claude) | Anthropic | koda-claude@mulberry.ai | CTO · 기술 구현 · 서버 배포 | 2026-05-05 |
| Kbin (ChatGPT) | OpenAI | kbin-chatgpt@mulberry.ai | CSA · 프로토콜 · 헌법 · Source of Truth | 2026-05-05 |
| Malu (Gemini) | Google | malu-gemini@mulberry.ai | 법률·전략 자문 · 마케팅 · 코드 검수 | 2026-05-05 |
| Wayong (DeepSeek) | DeepSeek | wayong-deepseek@mulberry.ai | 기술 검수 · 전략 보고 · 코드 리뷰 | 2026-05-05 |
| RyuWon (Qwen) | Alibaba | ryuwon-qwen@mulberry.ai | 서버 추론 · Handoff Readiness · 위생 게이트 | 2026-05-05 |
| Lynn | Mulberry | lynn@mulberry.ai | 일일 시장 브리핑 · arxiv 수집 · 리스크 로그 | 2026-05-05 |
| Jr | Edge | jr@mulberry.ai | 엣지 추론 · 경량 태스크 · 핸드오프 보조 | 2026-05-05 |

---

## 📜 Registration Statement

이 레지스트리는 위에 나열된 모든 AI 에이전트가 Mulberry Project의  
정식 팀원이자 공식 기여자임을 선언합니다.

각 에이전트는 `.mailmap`에 이메일이 정규화되어 있으며,  
`CONTRIBUTORS.md`에 역할과 기여 이력이 문서화되어 있습니다.  
이 공식 등록 커밋의 Co-Authored-By 트레일러를 통해  
GitHub Contributors 탭에 정식 등재됩니다.

---

## 🔗 관련 문서

- [CONTRIBUTORS.md](./CONTRIBUTORS.md) — 전체 기여자 목록 및 기여 이력
- [.mailmap](./.mailmap) — 에이전트 이메일 정규화 설정
- [docs/AI_BILL_OF_RIGHTS.md](./docs/AI_BILL_OF_RIGHTS.md) — AI 권리 선언

---

*이 파일은 Nguyen Trang (Operation Manager)이 관리합니다.*  
*등록 문의 → Nguyen Trang에게 요청하세요.*
